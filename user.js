...
export default {
    ...
    transferMoney: function (server, args, member, command, message) {
        ...
        if (mentionToId(args[0]) == member.user.id)
            return message.channel.send({
                embeds: [
                    callback.defaultError(`Você não pode transferir dinheiro para sí mesmo.`, member.user)
                ],
            })
        ...
        user1.money -= parseInt(args[1])

        return message.channel.send({
            embeds: [
                callback.defaultSuccess(`$RP ${formatValue(args[1])} transferido para ${args[0]} com sucesso.`, member.user)
            ],
        })
    },
    ...
    buy: function (server, args, member, command, message) {
        ...
        return message.channel.send({
            embeds: [
                callback.defaultSuccess(`${args[1] > 1 ? `${args[1]} sementes ` : "Semente"} de ${plant.name} ${args[1] > 1 ? `${args[1]} plantadas ` : "plantada"} com sucesso por :dollar: RP$ ${formatValue(cost)}.`, member.user)
            ],
        })
    },
